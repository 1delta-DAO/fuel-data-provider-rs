        ...
        loop {
            ...
            match TxSync::synchronize_transactions(1).await {
                ...
                Err(e) => {
                    ...
                    log::info!(
                        "Retrying sync service job in {} seconds",
                        calculate_backoff(retry_count)
                    );
                }
            }
            tokio::time::sleep(tokio::time::Duration::from_secs(calculate_backoff(retry_count)))
                .await;
        }
    });
        ...
        loop {
            ...
            match ExpiredDataManager::cleanup_job().await {
                ...
                Err(e) => {
                    ...
                    log::error!("Top level - Error occurred: {}", e);
                    log::info!(
                        "Retrying data cleanup job in {} seconds",
                        calculate_backoff(retry_count)
                    );
                }
            }
        ...
        loop {
            ...
            match CalculationManager::calculate_stats_job().await {
                ...
                Err(e) => {
                    ...
                    log::error!("Top level - Error occurred: {}", e);
                    log::info!(
                        "Retrying stats calculation job in {} seconds",
                        calculate_backoff(retry_count)
                    );
                }
            }
