    ...
    pub async fn calculate_stats_job() -> Result<(), DataException> {
        loop {
            ...
            for mut token in tokens {
                ...
                let volume_data_result =
                    VolumeDataService::find_by_token_id(&token.id).await.unwrap();
                if volume_data_result.is_some() {
                    let volume_data = volume_data_result.unwrap();
                    total_volume = volume_data.iter().map(|data| data.volume).sum();
                }

                log::info!("Token: {} - total volume: {:.2}", token.symbol, total_volume);
                token.volume_24_usd =
                    Converter::round_f64(total_volume * token.price, token.decimals);
                token.volume_24 = Converter::round_f64(total_volume, token.decimals);
                TokenService::update_volume(token.clone()).await.unwrap();

                // price_change_24

                let token_opening_price =
                    PriceDataService::find_oldest_by_token_id(&token.id).await.unwrap();
                if token_opening_price.is_some() {
                    let opening_price =
                        Converter::round_f64(token_opening_price.unwrap().price, token.decimals);
                    ...
                    let price_change24 =
                        (((current_price - opening_price) / opening_price) * 100.0) as f32;
                ...
                let found_token = liquidity
                    .iter()
                    .find(|token_liquidity| token_liquidity.address == token.address);
                match found_token {
                ...
                if token.quoting {
                    ...
                    token.liquidity_usd =
                        Converter::round_f64(token.liquidity * token.price, token.decimals);
                }
                ...
                if prices.len() < CONFIG.default.high_risk_swaps.into()
                    || token.liquidity_usd < CONFIG.default.high_risk_liquidity.into()
                {
