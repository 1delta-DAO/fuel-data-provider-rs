...
use crate::ports::db::repository::{
    CrudRepository, MiraPoolsRepository, TokenPairsRepository, TokenRepository,
};
use chrono::Utc;
...
impl MiraPoolsService {
    ...
    pub async fn update(pool_entity: MiraPoolsEntity) -> Result<MiraPoolsEntity, DbErr> {
        let mut active_model: crate::ports::db::model::mira_pools::ActiveModel =
            pool_entity.to_model().into();
    ...
    pub async fn collect_token_liquidity() -> Result<HashMap<Uuid, Decimal>, DbErr> {
        ...
        let pools_entities: Vec<MiraPoolsEntity> =
            all_pools.iter().map(|model| MiraPoolsEntity::from_model(model)).collect();

        // Get all token pairs to link pools with tokens
        let all_pairs = TokenPairsRepository::find_all().await?;
        let pairs_entities: Vec<TokenPairsEntity> =
            all_pairs.iter().map(|model| TokenPairsEntity::from_model(model)).collect();
        ...
        for pool in &pools_entities {
            ...
            if let Some(pair) = pair_map.get(&pool.pair_id) {
                ...
                let base_liquidity_entry =
                    token_liquidity.entry(base_token_id).or_insert(Decimal::ZERO);
                ...
                *base_liquidity_entry += pool.reserve_base.clone();
                ...
                let quote_liquidity_entry =
                    token_liquidity.entry(quote_token_id).or_insert(Decimal::ZERO);
    ...
    pub async fn prepare_tokens_liquidity() -> Result<Vec<TokenEntity>, DbErr> {
        ...
        for token_model in all_tokens {
            ...
            let raw_liquidity = token_liquidity.get(&token_id).cloned().unwrap_or(Decimal::ZERO);
            ...
            let liquidity_f64 =
                Converter::round_f64(liquidity.to_f64().unwrap_or(0.0), token_entity.decimals);
