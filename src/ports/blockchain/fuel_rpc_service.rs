...
impl FuelRpcService {
    ...
    pub async fn get_logs_by_block_number(
        ...
        for tx in block.transactions {
            if let Some(tx_response) = provider.get_transaction_by_id(&tx).await? {
                ...
                match tr {
                    TransactionType::Script(script_tx) => {
                        let mira_contract_id =
                            ContractId::from_str(CONFIG.default.cdi_mira_amm.as_str())?;
            ...
            tokio::time::sleep(std::time::Duration::from_millis(
                CONFIG.default.api_query_sleep_time.clone() as u64,
            ))
            .await;
        }
        if logs.len() > 0 {
    ...
    async fn get_logs_from_block_range(&self, block_number_start: u32, block_number_end: u32) {
        ...
        let results = stream::iter(block_number_start..=block_number_end)
            .map(|block_number| {
                let provider =
                    self.providers[block_number_start as usize % self.providers.len()].clone();
                async move {
                    match self.get_logs_by_block_number(&provider, block_number).await {
                        Ok(logs) => {
                            self.cache
                                .lock()
                                .unwrap()
                                .insert(block_number.to_string(), logs.clone());
                            Ok(logs)
                        }
                        Err(e) => {
        ...
        if logs.len() > 0 {
            ...
            if let Some(cached_block) = latest_cached_block {
                if requested_block <= cached_block
                    && requested_block >= first_cached_block.unwrap_or(0)
                {
    ...
    fn get_latest_cached_block(&self) -> Option<u32> {
        ...
        cache.keys().map(|k| k.parse::<u32>().unwrap_or(0)).max()
    }
    fn get_first_cached_block(&self) -> Option<u32> {
        let cache = self.cache.lock().unwrap();
        cache.keys().map(|k| k.parse::<u32>().unwrap_or(0)).min()
    }
